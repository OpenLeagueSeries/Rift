...
export function nameValidator(event) {
  const defaultHelperText = ['Your first and last name here', 'Looks good!'];
  ...
  this.setState(
    ...
    () => {
      ...
      if (validator.matches(this.state.name, /[ ]{2,}/)) {
        this.setState(state => ({
          NameHelperText: 'Name contains multiple spaces',
          inputError: true,
          nextForm: false
        }));
      }
      if (this.state.name.charAt(0) === ' ') {
        this.setState(state => ({
          NameHelperText: 'Name begins with a space',
          inputError: true,
          nextForm: false
        }));
      }
      if (
        ...
        const NameParts = this.state.name
          .split('-')
          .map(
          ...
          .join(' ');
        this.setState(state => ({
          NameHelperText: defaultHelperText[1],
          name: NameParts.replace(/  +/g, ' '),
          inputError: false,
          nextForm: true
        }));
      }
...
export const NameField = props => {
  ...
  const disableButton =
    ...
    validator.matches(props.name, /[ ]{2,}/) ||
    props.name.charAt(0) === ' ';
